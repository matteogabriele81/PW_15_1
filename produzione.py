...
class Produzione:
    ...
    def __init__(self, config: ConfigurazioneOrtaggio, metri_quadri: float, 
        ...
        # Capacità giornaliera per la competizione tra le colture (None = usa il valore di configurazione)
        self.capacita_effettiva = None
    ...
    def calcola_produzione(self):
        ...
        tener conto della manodopera. Si badi che la capacità è vincolata dalla massima
        capacità di lavorazione giornaliera, se impostata, sennò si usa quella di configurazione.
        """
        capacita = self.capacita_effettiva if self.capacita_effettiva is not None else self.config.capacita_lavorazione
        giorni_lavorazione = self.metri_quadri / capacita 
