...
Introduce anche un vincolo di competizione tra colture: la capacità viene allocata in base
a una regola (resa_base decrescente), cambiando i tempi di lavorazione.
"""
...
CAPACITA_GIORNALIERA_TOTALE = 300  # mquadri/giorno per il calcolo della competizione tra colture
...
class SimulatoreFattoria:
    ...
    def simula_produzione(self, produzioni: List[Produzione], 
        ...
        # Applica la funzione di competizione tra colture prima di calcolare la produzione
        self._applica_competizione(produzioni)
        ...
        for i, produzione in enumerate(produzioni, 1):
            ...
            print(f"    Capacità giornaliera: {produzione.capacita_effettiva:.0f}/{produzione.config.capacita_lavorazione:.0f} m²/giorno")
    ...
    def _applica_competizione(self, produzioni: List[Produzione]):
        """
        Modifica della capacità giornaliera condivisa alle colture in ordine di priorità.
        La priorità è determinata dalla resa_base decrescente: chi produce di più
        per metro quadro ha diritto a più risorse giornaliere e quindi alla raccolta.
        Le colture meno prioritarie ottengono la capacità residua, l'effetto è che si allunga
        il numero di giorni di lavorazione necessari.
        """
        in_ordine = sorted(produzioni, key=lambda p: p.config.resa_base, reverse=True)
        
        print(f"  Competizione per capacità giornaliera totale: {CAPACITA_GIORNALIERA_TOTALE} mquadri/giorno")
        print(f"  Priorità (resa base decrescente): " +
              " > ".join(p.config.tipo.value for p in in_ordine))
        print()
        
        rimanente = CAPACITA_GIORNALIERA_TOTALE
        for p in in_ordine:
            assegnata = min(p.config.capacita_lavorazione, max(rimanente, 10))
            p.capacita_effettiva = assegnata
            rimanente -= assegnata
